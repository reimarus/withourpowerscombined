-- aiarchetype-managerloader.lua — WOPC patched copy of vanilla
-- Wraps SetupUnderEnergyStatTrigger / SetupUnderMassStatTrigger in pcall
-- because the FAF-patched exe may not expose these C++ brain methods.
...
function ExecutePlan(aiBrain)
    ...
    if not aiBrain.BuilderManagers.MAIN.FactoryManager:HasBuilderList() then
        ...
        -- WOPC: wrapped in pcall — these engine methods may not exist
        -- in the FAF-patched binary
        pcall(aiBrain.SetupUnderEnergyStatTrigger, aiBrain, 0.1)
        pcall(aiBrain.SetupUnderMassStatTrigger, aiBrain, 0.1)
