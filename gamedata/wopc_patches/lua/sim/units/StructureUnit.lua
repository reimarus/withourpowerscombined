...
local function GetStructureUnitRotateTowardsEnemiesLand() return categories.STRUCTURE + categories.LAND + categories.NAVAL end
-- Structures that rotate towards the enemy: PD, AA, Torp launchers, Artillery, and TMD (except Volcano)
local function GetStructureUnitOnStartBeingBuiltRotateBuildings() return categories.STRUCTURE * (categories.DIRECTFIRE + categories.ANTIAIR + categories.ANTINAVY + categories.ARTILLERY + (categories.ANTIMISSILE - categories.SILO - categories.uab4201)) end
-- Structures that rotate in 90 degree steps: T2/T3/T4 Artillery
local function GetStructureUnitRotateStaticArty() return categories.ARTILLERY * (categories.TECH2 + categories.TECH3 + categories.EXPERIMENTAL)
end
-- These always point towards map center and only rotate at 90degree steps
...
StructureUnit = ClassUnit(Unit, BlinkingLightsUnitComponent) {
    ...
    OnCreate = function(self)
        ...
        if EntityCategoryContains(GetStructureUnitOnStartBeingBuiltRotateBuildings(), self) then
    ...
    RotateTowardsEnemy = function(self)
        ...
        if not EntityCategoryContains(GetStructureUnitRotateStaticArty(), self) then
            -- determine radius
            local radius = 40
            ...
            local cats = EntityCategoryContains(categories.ANTIAIR, self) and categories.AIR or (GetStructureUnitRotateTowardsEnemiesLand())
        ...
        if bpFootprint then
            ...
            elseif EntityCategoryContains(GetStructureUnitRotateStaticArty(), self) then
