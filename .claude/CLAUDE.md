...
13. **Ask clarifying questions proactively.** When requirements are ambiguous, ask before implementing. Questions save time and prevent rework. Don't assume — confirm. The user appreciates good questions.
14. **SCFA engine team numbering: UI N → engine N+1.** The engine uses Team 1 = no team (FFA), Team 2+ = allied teams. The UI shows teams 1-4 for user friendliness. Always add 1 when passing team values to `game_config.py` or `quickstart.lua`. This offset caused incorrect team alliances in game.
